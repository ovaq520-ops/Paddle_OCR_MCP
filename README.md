...
识别图片中的所有文字。**通常无需传参**，系统通过 `CLAUDE_CODE_SESSION_ID` 自动定位当前会话 transcript，不会在多窗口场景下串图。

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `image_path` | string | 否 | 图片文件完整路径，不传则自动从 transcript 提取 |
| `transcript_path` | string | 否 | 手动指定 transcript 文件路径（仅调试用，通常无需传） |
...
| `CLAUDE_PROJECTS_ROOT` | `~/.claude/projects` | transcript 搜索根目录 |
| `CLAUDE_CODE_SESSION_ID` | Claude Code 自动注入 | 当前会话 ID，用于精确定位 transcript，避免多窗口串图 |

> **会话定位原理**：`CLAUDE_CODE_SESSION_ID` 的值即为当前会话 transcript 的文件名（如 `xxx.jsonl`），MCP 服务通过精确文件名匹配直接定位，不再依赖文件修改时间的启发式猜测。
